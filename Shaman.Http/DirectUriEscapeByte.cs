...
namespace Shaman.Runtime
{
    class DirectUriEscapeByte
    {
        ...
        internal unsafe static byte[] EscapeString(string input, int start, int end, byte[] dest, ref int destPos,
                ...
                for (; i < end; ++i)
                {
                    ...
                    else if (ch == '%' && rsvd == '%')
                    {
                        ...
                        if (i + 2 < end && EscapedAscii(pStr[i + 1], pStr[i + 2]) != c_DummyChar)
                        {
                            // leave it escaped
                            dest[destPos++] = (byte)'%';
                            dest[destPos++] = (byte)pStr[i + 1];
                            dest[destPos++] = (byte)pStr[i + 2];
                            i += 2;
                        }
                        else
                        {
        ...
        [AllowNumericLiterals]
        internal static void EscapeAsciiChar(char ch, byte[] to, ref int pos)
        {
            if (ch == ' ')
            {
                to[pos++] = (byte)'+';
        ...
        private static readonly byte[] HexUpperChars = {
        ...
        //
        private unsafe static byte[] EnsureDestinationSize(char* pStr, byte[] dest, int currentInputPos,
            short charsToAdd, short minReallocateChars, ref int destPos, int prevInputPos)
        {
            if ((object)dest == null || dest.Length < destPos + (currentInputPos - prevInputPos) + charsToAdd)
            {
                ...
                byte[] newresult = new byte[destPos + (currentInputPos - prevInputPos) + minReallocateChars];

                if ((object)dest != null && destPos != 0)
                    Buffer.BlockCopy(dest, 0, newresult, 0, destPos);
                dest = newresult;
            }

            // ensuring we copied everything form the input string left before last escaping
            while (prevInputPos != currentInputPos)
                dest[destPos++] = (byte)pStr[prevInputPos++];
            return dest;
        }
